...
namespace BusinessLogic::Test
{
    ...
    protected:
        void SetUp() override
        {
            ...
            // Create reference arrays using new with aggregate initialization
            sources = std::unique_ptr<std::array<std::reference_wrapper<Device::IMeasurementSource>, 2>>(
                new std::array<std::reference_wrapper<Device::IMeasurementSource>, 2>{{std::ref(*mockSource1),
                                                                                       std::ref(*mockSource2)}});

            recorders = std::unique_ptr<std::array<std::reference_wrapper<Device::IMeasurementRecorder>, 2>>(
                new std::array<std::reference_wrapper<Device::IMeasurementRecorder>, 2>{{std::ref(*mockRecorder1),
                                                                                         std::ref(*mockRecorder2)}});

            // Create coordinator
            coordinator = std::make_unique<MeasurementCoordinator<2, 2>>(*sources, *recorders);
        }
        ...
        MeasurementCoordinator<2, 2> *getCoordinator() { return coordinator.get(); }
    ...
    private:
        ...
        std::unique_ptr<MeasurementCoordinator<2, 2>> coordinator;
    };
    ...
    protected:
        ...
        void SetUp() override
        {
            ...
            // Create arrays using new with aggregate initialization
            sources = std::unique_ptr<std::array<std::reference_wrapper<Device::IMeasurementSource>, NUM_SOURCES>>(
                new std::array<std::reference_wrapper<Device::IMeasurementSource>, NUM_SOURCES>{{std::ref(*mockSources[0]),
                                                                                                 std::ref(*mockSources[1]),
                                                                                                 std::ref(*mockSources[2]),
                                                                                                 std::ref(*mockSources[3]),
                                                                                                 std::ref(*mockSources[4])}});

            recorders = std::unique_ptr<std::array<std::reference_wrapper<Device::IMeasurementRecorder>, NUM_RECORDERS>>(
                new std::array<std::reference_wrapper<Device::IMeasurementRecorder>, NUM_RECORDERS>{{std::ref(*mockRecorders[0]),
                                                                                                     std::ref(*mockRecorders[1]),
                                                                                                     std::ref(*mockRecorders[2])}});

            coordinator = std::make_unique<MeasurementCoordinator<NUM_SOURCES, NUM_RECORDERS>>(*sources, *recorders);
        }
        ...
        MeasurementCoordinator<NUM_SOURCES, NUM_RECORDERS> *getCoordinator() { return coordinator.get(); }
    ...
    private:
        ...
        std::unique_ptr<MeasurementCoordinator<NUM_SOURCES, NUM_RECORDERS>> coordinator;
    };
