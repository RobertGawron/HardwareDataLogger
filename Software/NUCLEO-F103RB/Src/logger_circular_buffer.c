...
} logger_circular_buffer_t;

static logger_circular_buffer_t buffer;
...
void Logger_CircularBuffer_Insert(GMMeasurement_Value_t element)
{
    ...
    buffer.indexOfNextToInsert %= LOGGER_CIRCULAR_BUFFER_MAX_ELEMENTS;

    if(buffer.elementsInBuffer < LOGGER_CIRCULAR_BUFFER_MAX_ELEMENTS)
    {
...
Logger_CircularBuffer_GetElementStatus_t Logger_CircularBuffer_GetElement(GMMeasurement_Value_t* element, uint16_t index)
{
    ...
    if(index < LOGGER_CIRCULAR_BUFFER_MAX_ELEMENTS)
    {
        int16_t localIndex = (int16_t)( (int16_t)(buffer.indexOfNextToInsert - 1) - (int16_t)index );
        if(localIndex < 0)
        {
            localIndex = (int16_t)(LOGGER_CIRCULAR_BUFFER_MAX_ELEMENTS + localIndex);
        }

        *element = buffer.data[localIndex];
        retValue = LOGGER_CIRCULAR_BUFFER_GETELEMENT_OK;
    }
    else
    {
        *element = 0;
        retValue = LOGGER_CIRCULAR_BUFFER_GETELEMENT_INDEX_OUT_OF_RANGE;
    }
...
Logger_CircularBuffer_GetMinMaxElementStatus_t Logger_CircularBuffer_GetMinMaxElement(GMMeasurement_Value_t* minElement, GMMeasurement_Value_t* maxElement)
{
    ...
    if(Logger_CircularBuffer_GetElementCount() != 0u)
    {
        Logger_CircularBuffer_GetElement(minElement, 0);
        Logger_CircularBuffer_GetElement(maxElement, 0);
        ...
        retValue = LOGGER_CIRCULAR_BUFFER_GETMINMAXELEMENT_OK;
    }
    else
    {
        ...
        *minElement = 0;
        *maxElement = 0;
        retValue = LOGGER_CIRCULAR_BUFFER_GETMINMAXELEMENT_NO_MEASSUREMENTS;
    }
