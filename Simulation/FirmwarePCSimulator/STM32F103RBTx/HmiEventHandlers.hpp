...
// using SerialTxCallback = std::function<HAL_StatusTypeDef(const std::uint8_t *, std::uint16_t, std::uint32_t)>;
typedef int (*SerialTxCallback)(const uint8_t *pData, uint16_t size, uint32_t timeout);
