...
#include "WString.h"
// hacks ========================================================================
// #include "stdlib_noniso.h" rgawron

#define _OPEN_SYS_ITOA_EXT
#include <stdlib.h>

#define strlen_P std::strlen // changed, toolchain-xtensa\xtensa-lx106-elf\include\sys\string.h
#define PGM_P const char *   // toolchain-xtensa\xtensa-lx106-elf\include\sys\pgmspace.h

#include <cstring>
#define memmove_P std::memmove;
#define memcpy_P std::memcpy;
// #define strstr_P std::strstr;

#include <cstdio>
#include <cstdlib>
#include <cstring>

// Implementation of strstr_P
// Searches for the first occurrence of s2 in buffer starting from a specific index
const char *strstr_P(const char *buffer, const char *s2)
{
    if (!buffer || !s2)
    {
        return nullptr; // Ensure inputs are not null
    }

    return strstr(buffer, s2); // Use standard strstr for implementation
}

//=====================================================================================

// Implementation of utoa using sprintf
char *utoa(unsigned int value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%u" : "%x"), value);
    return buf;
}

// Implementation of itoa using sprintf
char *itoa(int value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%d" : "%x"), value);
    return buf;
}

// Implementation of ltoa using sprintf
char *ltoa(long value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%ld" : "%lx"), value);
    return buf;
}

// Implementation of ultoa using sprintf
char *ultoa(unsigned long value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%lu" : "%lx"), value);
    return buf;
}

// Implementation of lltoa using sprintf
char *lltoa(long long value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%lld" : "%llx"), value);
    return buf;
}

// Implementation of ulltoa using sprintf
char *ulltoa(unsigned long long value, char *buf, int base)
{
    ...
    sprintf(buf, (base == 10 ? "%llu" : "%llx"), value);
    return buf;
}

// Implementation of lltoa with buffer size using snprintf
char *lltoa(long long value, char *buf, size_t buf_size, int base)
{
    if (base < 2 || base > 36 || buf_size == 0)
    {
        if (buf_size > 0)
            *buf = '\0'; // Invalid base or buffer size
        return buf;
    }
    snprintf(buf, buf_size, (base == 10 ? "%lld" : "%llx"), value);
    return buf;
}

// Implementation of ulltoa with buffer size using snprintf
char *ulltoa(unsigned long long value, char *buf, size_t buf_size, int base)
{
    if (base < 2 || base > 36 || buf_size == 0)
    {
        if (buf_size > 0)
    ...
    snprintf(buf, buf_size, (base == 10 ? "%llu" : "%llx"), value);
    return buf;
}
...
// Converts a double value to a string with specified width and precision
char *dtostrf(double value, int width, unsigned int precision, char *buf)
{
    if (!buf)
    {
        return nullptr; // Ensure buffer is not null
    }

    // Format string to ensure proper width and precision
    char format[16];
    snprintf(format, sizeof(format), "%%%d.%df", width, precision);

    // Perform the conversion
    snprintf(buf, width + precision + 2, format, value);
...
String::String(unsigned char value, unsigned char base) : String(toString(value, base))
{
}

String::String(int value, unsigned char base) : String(toString(value, base))
{
}

String::String(unsigned int value, unsigned char base) : String(toString(value, base))
{
}

String::String(long value, unsigned char base) : String(toString(value, base))
{
}

String::String(unsigned long value, unsigned char base) : String(toString(value, base))
{
}

String::String(long long value, unsigned char base) : String(toString(value, base))
{
}

String::String(unsigned long long value, unsigned char base) : String(toString(value, base))
{
}

String::String(float value, unsigned char decimalPlaces) : String(toString(value, decimalPlaces))
{
}

String::String(double value, unsigned char decimalPlaces) : String(toString(value, decimalPlaces))
{
}
