...
public:
    ...
    explicit String(unsigned char value) : String(value, 10)
    {
    }

    String(int, unsigned char base);
    explicit String(int value) : String(value, 10)
    {
    }

    String(unsigned int, unsigned char base);
    explicit String(unsigned int value) : String(value, 10)
    {
    }

    String(long, unsigned char base);
    explicit String(long value) : String(value, 10)
    {
    }

    String(unsigned long, unsigned char base);
    explicit String(unsigned long value) : String(value, 10)
    {
    }

    String(long long, unsigned char base);
    explicit String(long long value) : String(value, 10)
    {
    }

    String(unsigned long long, unsigned char base);
    explicit String(unsigned long long value) : String(value, 10)
    {
    }

    String(float, unsigned char decimalPlaces);
    explicit String(float value) : String(value, 2)
    {
    }

    String(double, unsigned char decimalPlaces);
    explicit String(double value) : String(value, 2)
    {
    }
    ...
    [[nodiscard]] unsigned int length(void) const
    {
        return buffer() ? len() : 0;
    }
    void clear(void)
    ...
    bool operator<=(const String &rhs) const;
    bool operator>=(const String &rhs) const;
    [[nodiscard]] bool equalsIgnoreCase(const String &s) const;
    bool equalsIgnoreCase(const __FlashStringHelper *s) const;
    [[nodiscard]] unsigned char equalsConstantTime(const String &s) const;
    ...
    [[nodiscard]] char *begin() { return wbuffer(); }
    [[nodiscard]] char *end() { return wbuffer() + length(); }
    [[nodiscard]] const char *begin() const { return c_str(); }
    [[nodiscard]] const char *end() const { return c_str() + length(); }

    // search
    [[nodiscard]] int indexOf(char ch, unsigned int fromIndex = 0) const;
    [[nodiscard]] int indexOf(const char *str, unsigned int fromIndex = 0) const;
    [[nodiscard]] int indexOf(const __FlashStringHelper *str, unsigned int fromIndex = 0) const
    {
        return indexOf((const char *)str, fromIndex);
    }
    [[nodiscard]] int indexOf(const String &str, unsigned int fromIndex = 0) const;
    ...
    [[nodiscard]] int lastIndexOf(const __FlashStringHelper *str) const;
    [[nodiscard]] int lastIndexOf(const __FlashStringHelper *str, unsigned int fromIndex) const;
...
protected:
    ...
    // Accessor functions
    [[nodiscard]] bool isSSO() const { return !sso.isHeap; }
    [[nodiscard]] unsigned int len() const { return isSSO() ? sso.len : ptr.len; }
    [[nodiscard]] unsigned int capacity() const { return isSSO() ? (unsigned int)SSOSIZE - 1 : ptr.cap; } // Size of max string not including terminal NUL
